...
def main(argv: list[str] | None = None) -> int:
    ...
    def on_commit(t0: float, t1: float, en: str, ja: str) -> None:
        print(f"[utt {t0:.2f}-{t1:.2f}] EN: {en}")
        print(f"[utt {t0:.2f}-{t1:.2f}] JA: {ja}")

    handler = LiveASRTranslateHandler(translator=translator, on_commit=on_commit)

    runner = LiveUtteranceTranscriber(
        chunk_iter=mic.chunks(),
        transcriber=transcriber,
        vad=vad,
        on_asr=handler.handle_asr,
    ...
    print("ItoSub live ASR->translation (utterance mode). Press Ctrl+C to stop.")
    try:
        runner.run()
